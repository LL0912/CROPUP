...
with open("./config/model/unite_alda.yaml") as f:
...
ad_net = Multi_AdversarialNetwork(cfg_m["network"]["features_dim"], cfg_m["network"]["hidden_size"], cfg_d["Model"]["num_classes"])
ad_net.to(device)
model.to(device)
...
test_metric_all=train_model(seed,cfg_m,model,ad_net,model_start_path,EPOCH,optimizer,lr_scheduler,criterion,device,
