...
from utils.helper_unite_dann import *
from utils.io_func import save_dict_to_excel
from utils.io_func import make_parent_dir_list
from torch.optim.lr_scheduler import LambdaLR
from torch.optim.sgd import SGD
from models.dann import DomainDiscriminator
from loss.dann_loss import DomainAdversarialLoss
...
with open("./config/model/unite_dann.yaml") as f:
...
header={"loss":["loss"],"overall":["overall"],"kappa":["kappa"],"recall":category,"precision":category,"f1":category,"Confusion matrix":category}
...
domain_discri = DomainDiscriminator(cfg_m["network"]["features_dim"], hidden_size=cfg_m["network"]["hidden_size"]).to(device)
...
test_metric_all=train_model(seed,cfg_m,model,domain_adv,model_start_path,EPOCH,optimizer,lr_scheduler,criterion,device,
            train_loader_source,train_loader_target,test_loader_target,steps_per_epoch,
